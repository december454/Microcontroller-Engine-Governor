...
// Engine Governor: State-Based Design - LCD Output Rewrite + Ammeter

const int ammeterPin = A15;                   // Pin for ammeter.
...
const int ampPrecision = 13;                   // Acceptable discrepency between nominal and current amperage (0 - 1024).
const int ampCheckThreshold = 3;             // Number of abnormal amperage readings before action is taken.
...
boolean ampSpike = false;             // If the amperage has suddly increased or decreased.
...
int nominalAmps = 0;          // Recent and normal amperage reading.
int ampChecks = 0;            // Number of abnormal amperage readings.
int ampDiff = 0;              // Difference between the current amperage and nominal amperage.
...
void setup() {
  ...
  Serial.begin(9600);
  nominalAmps = analogRead(ammeterPin);
}
...
void adjustThrottle(){
  ...
  if (ampSpike){
    stepsRemaining = (int)abs(ampDiff*1.1);
  }

  if (stepsRemaining > 0){
    boolean directionFlag = false;
    if (rpmDiff < 0 || ampDiff < 0)
      directionFlag = true;
...
void checkAmmeter(){
  int currentAmps = analogRead(ammeterPin);
  ampDiff = (nominalAmps - currentAmps);

  if (abs(ampDiff) < ampPrecision){
    ampSpike = false;
    nominalAmps = currentAmps;
    ampChecks = 0;
  }
    ...
    if (ampChecks >= ampCheckThreshold){
      ampSpike = true;
      nominalAmps = currentAmps;
      ampChecks = 0;
      adjustThrottle();
      Serial.print("Amperage Spike!");
